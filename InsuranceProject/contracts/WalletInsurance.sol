...
contract WalletInsurance {
    ...
    constructor(uint256 _insuredAmount) {
        // Set the contract owner as the transaction origin
        owner = tx.origin; 
    ...
    function payInsurance() external payable {
        ...
        require(
            msg.value >= insuredAmount, 
            "The amount provided is not valid."); 

        // Check if the payment is allowed at this time
        require(
            block.timestamp > insuranceDuration, 
            "You are unable to make a payment at this time."); 
    ...
    function claimInsurance() external payable onlyOwner() {
        ...
        require(
            block.timestamp > insuranceDuration, 
            "Your insurance is still valid and has not expired."); 
